...
int main() {
    try {
        ...
        while (true) {
            ...
            // 只检查HTTP状态码判断登录是否成功
            if (choice == 2 && status_code == 200) {
                std::cout << "登录成功" << std::endl;
                
                // 获取用户ID（如果服务端返回了用户ID）
                if (response_json.contains("user_id") && response_json["user_id"].is_string()) {
                ...
                while (true) {
                    ...
                    if (choice == 1) {
                        ...
                        post_path = "/api/post/publish";
                        
                    } else if (choice == 2) {
                        // 删帖
                        std::string post_id;
                        std::cout << "请输入帖子ID: ";
                        std::getline(std::cin, post_id);
                        
                        post_data["post_id"] = post_id; // 添加帖子ID到消息体
                        post_path = "/api/post/delete";
                    ...
                    // 发送请求
                    auto [post_status, post_response] = sendHttpRequest(client, "POST", post_path, post_data);
